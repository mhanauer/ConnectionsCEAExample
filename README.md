...
ER and Hosp WISQARS age range 21 to 67 everything else included. Took the average of the five intents that it included: unitentiaional, sexual assualt, other assualt, self-harm, legal intervention: https://wisqars.cdc.gov:8443/costT/cost_Part1_Finished.jsp This include work loss costs not sure if that should be included have a low rate of working.
...
##Inpat / Outpat costs: https://www.kff.org/health-costs/state-indicator/expenses-per-inpatient-day/?currentTimeframe=0&selectedRows=%7B%22states%22:%7B%22indiana%22:%7B%7D%7D%7D&sortModel=%7B%22colId%22:%22Location%22,%22sort%22:%22asc%22%7D

Here, we used $2,508 (2017 dollars) from the KFF data that combined in-patient and out-patient cost data and combined the in-patient and out-patient data in our analysis to be consistent with the data. This is specifically for Indiana, where WISQARS is nationwide data. **Future projects should look to revising survey questions to be more tailored towards using the CDC WISQARS data.**
...
**Total In-patient and Out-patient Nights for base and month 6**
...
print(total_pat_base <- sum(Total_inpat_base,Total_outpat_base))
```
...
print(total_pat_month6 <- sum(Total_inpat_month6, Total_outpat_month6))
```

Within the 6 month timeframe: Total visits went from 253 to 116; in-patient visits went from 80 to 46; out-patient visits went from 173 to 70. **Future studies should look to try and tease apart the cost difference between in-patient and out-patient costs.**

###Part I - Hospitalization Savings

```{r}
print(Part1_HospSavings <- (total_pat_base - total_pat_month6)*HospCosts) 
```

The program saved $352,138 in the sample.

###Part II - Hospitalization Savings

```{r}
print(Part2i_HospSavings<-Part1_HospSavings*1.445)
print(Part2ii_HospSavings<-Part1_HospSavings + (Part1_HospSavings*0.50*0.445))
print(Part2iii_HospSavings<-Part1_HospSavings + (Part1_HospSavings*0.25*0.445))
```

The program saved about $508,840.
