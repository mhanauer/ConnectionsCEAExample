...
In order to asses the benefits and costs of this program, we split our analysis into two parts. Part I, we analyzed only the benefits and costs of the sample. Part II, we predict the benefits and costs of the program as a whole. We analyzed 3 different scenarios for Part II. All scenarios will be based off of the benefits and costs that were calculated analyzing the data in the sample. To assess the full benefits and costs of the program, our sample only contained responses from individuals that filled out both the intake and 6-month survey. In our study, that was 105 out of 186 (56.5% of participants). The remaining 81 individuals completed the intake survey and either finished the program and did not fill out the 6-month survey, or dropped out of the program sometime within the 6-month timeframe. To account for some this variation the following 3 scenarios will be analyzed: Scenario 1, will predict the benefits and costs of the program by taking the total net benefits/costs of each variable in the sample and assuming that this would be the same for the remaining 81 participants (43.5%) in the program. Scenario 2, will assume a 50% decrease in observed benefits and costs for the participants that did not complete the survey. Scenario 3, will assume a 75% decrease in observed benefits and costs of the unobserved particpants. We are assuming that the program spent all 2 million dollars of the grant money that was awarded to the program, and that funding was equally spreadout among all participants in the program, regardless of retention status. 
...
**Where to find the most up-to-date inflation rates:** 
1) Go to https://apps.bea.gov/iTable/iTable.cfm?reqid=19&step=2#
2) Click on "Section 2 - PERSONAL INCOME AND OUTLAYS"
3) Select "Table 2.3.3 - Table 2.3.3 - Real Personal Consumption Expenditures by Major Type of Product, Quantity Indexes"
4) Go to "Healthcare" under the "Services" section
5) Average the inflation rate over the 4 quarters
...
Since we were able to analyze 56.5% of the particpants in the study, that leaves us with 43.5% that is unaccounted. In order to better estimate the impact of the whole program we need to predict the benefits/costs of those were not analyzed. In the following calculations for Part II, the benefits/costs by using **1.435** (Scenario 1) or **0.435** (Scenarios 2 & 3). 

##ER 

**Question(s):**

During the past 30 days did you receive Emergency Room Treatment for: Mental or emotional difficulties? If yes, altogether how many times?

During the past 30 days, did you receive Emergency Room Treatment for: Alcohol or substance abuse? If yes, altogether how many times?
...
```{r}
print(Part2i_ERsavings<- Part1_ERsavings*1.435)
print(Part2ii_ERsavings<- Part1_ERsavings + (Part1_ERsavings*0.5*0.435))
print(Part2iii_ERsavings<- Part1_ERsavings + (Part1_ERsavings*0.25*0.435))
```

In Part I, the total savings came out to be $27,888. In Part II, Scenario 1 showed cost savings of $40,019; Scenario 2 showed cost savings of $33,954; and Scenario 3 showed cost savings of $30,921. 

##In-patient Costs

**Question(s):**

During the past 30 days, did you receive Inpatient Treatment for: Mental or emotional difficulties? If yes, altogether how many nights?

During the past 30 days did you receive Inpatient Treatment for: Alcohol or substance abuse? If yes, altogether how many nights?
...
```{r}
print(Part2i_InpatSavings<-Part1_InpatSavings*1.435)
print(Part2ii_InpatSavings<-Part1_InpatSavings + (Part1_InpatSavings*0.50*0.435))
print(Part2iii_InpatSavings<-Part1_InpatSavings + (Part1_InpatSavings*0.25*0.435))
```

The program saved about $125,407 in Scenario 1, $106,400 in Scenario 2, and $96,896 in Scenario 3.

##Out-patient Costs

**Question(s):**

During the past 30 days, did you receive Outpatient Treatment for: Mental or emotional difficulties? If yes, altogether how many times?

During the past 30 days, did you receive Outpatient Treatment for: Alcohol or substance abuse? If yes, altogether how many times?
...
```{r}
print(Part2i_OutpatSavings<-Part1_OutpatSavings*1.435)
print(Part2ii_OutpatSavings<-Part1_OutpatSavings + (Part1_OutpatSavings*0.50*0.435))
print(Part2iii_OutpatSavings<-Part1_OutpatSavings + (Part1_OutpatSavings*0.25*0.435))
```

The program saved about $44,342 in Scenario 1, $37,621 in Scenario 2, and $34,260 in Scenario 3.
...
Approximately, how much money did YOU receive (pre-tax individual income) in the past 30 days from … Wages.
...
Approximately, how much money did YOU receive (pre-tax individual income) in the past 30 days from …Public assistance.
...
Approximately, how much money did YOU receive (pre-tax individual income) in the past 30 days from …Disability.
...
```{r}
print(Part2i_Income<-Part1_Income*1.435)
print(Part2ii_Income<-Part1_Income + (Part1_Income*0.5*0.435))
print(Part2iii_Income<-Part1_Income + (Part1_Income*0.25*0.435))
```

Scenario 1 had a $25,510 increase in income, Scenario 2 had a $21,644 increase, and Scenario 3 had a $19,710 increase.
...
Incarceration costs came from the Prisons Bureau. The cost per day to house someone is $99.45 (2017 dollars) and then was $101.88 after adjusting for inflation.
...
###Part II
```{r}
print(Part2i_Incarceration<- Part1_Incarceration*1.435)
print(Part2ii_Incarceration<- Part1_Incarceration + (Part1_Incarceration*0.5*0.435))
print(Part2ii_Incarceration<- Part1_Incarceration + (Part1_Incarceration*0.25*0.435))
```

Adds $29,824 to the program costs in Scenario 1, $25,304 in Scenario 2, and $23,044 in Scenario 3.
...
$58 per person in opportunity cost and would be $6,090 in overall program costs in the sample.
